...
interface MapStore {
  ...
  setMapProvider: (provider: "leaflet") => void
  toggleSidebar: () => void
...
export const useMapStore = create<MapStore>((set, get) => ({
  ...
  initializeLayers: () => {
    console.log("📋 MapStore: initializeLayers called")
    try {
      const currentState = get()
      console.log("📋 MapStore: Current layers length:", currentState.layers.length)
      ...
      if (currentState.layers.length > 0) {
        ...
        console.log("📋 MapStore: Current layers:", currentState.layers.map(l => ({ id: l.id, active: l.active })))
        return
      }
      ...
      const initialLayers: MapLayer[] = [
        ...
        },
        {
          id: "state-land",
          name: "State Land",
          description: "State land areas with fards and pictures",
          active: false,
          color: "#FFD700", // Gold color for state land
          type: "polygon",
          geojsonPath: "/data/state-land.geojson", // This will be provided by user
          icon: "MapPin",
        },
        {
          id: "state-office",
          name: "State Offices",
          description: "Government state office locations",
          active: false,
          color: "#DC2626", // Red color for state offices
          type: "point",
          geojsonPath: "/data/state-offices.geojson", // This will be provided by user
          icon: "Building2",
        },
      ]
      ...
      set({
        ...
        layerVersion: currentState.layerVersion + 1
      })
      ...
      setTimeout(() => {
        const newState = get()
        console.log("📋 MapStore: Verification - layers after init:", newState.layers.length)
      }, 100)
      
    } catch (error) {
      console.error("📋 MapStore: Error initializing layers:", error)
    }
  },
  ...
  setMapProvider: (provider: "leaflet") => {
    try {
      set((state) => {
        const newVersion = state.layerVersion + 1
